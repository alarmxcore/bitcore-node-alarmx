...
To develop bitcore-node-alarmx:
...
We now will setup symlinks in `bitcore-node-alarmx` *(repeat this for any other modules you're planning on developing)*:
```bash
cd node_modules
rm -rf bitcore-lib
ln -s ~/bitcore-lib
rm -rf bitcoind-rpc-alarmx
ln -s ~/bitcoind-rpc-alarmx
```
...
cd devnode
mkdir node_modules
touch bitcore-node-alarmx.json
touch package.json
```

Edit `bitcore-node-alarmx.json` with something similar to:
```json
{
  "network": "livenet",
  "port": 3001,
...
```bash
../bitcore-node-alarmx/bin/bitcore-node-alarmx start
```
